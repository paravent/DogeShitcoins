...
async function boot() {
	...
	await type("Welcome to DOGE Mainframe login", {
		initialWait: 3000
	});

	await type(
		[
			"> SET DOGE_HACK;)/INITIAISE",
			"Loading...................kek"
		],
		{
			lineWait: 1000
	...
	await type(
		[
			"> IF ONLY THERE WAS A WAY ONLY DOGS COULD FIND WHAT IM LOOKING FOR"
		],
		{
			lineWait: 1000,
			finalWait: 3000
...
async function login() {
	...
	if (user === USER && password === PW) {
		await pause();
		say("DOGETHENTICATION SUCCESSFUL");
		await alert("DOGETHENTICATION SUCCESSFUL");
		clear();
		return (window.location.href = "https://dexscreener.com/");
	} else {
		await type([
			"Incorrect user and/or password.",
			"Please try again KEK"
		]);
		await pause(3);
		clear();
		return login();
